...
class Trainer:
    def __init__(self, configs):
        ...
        self.network = AMBHFN_Model().to(configs.device)
    ...
    def train_once(self, input_uv, uv_true, ssr_ratio, ele):
        ...
        ele = ele.float().to(self.device)

        for i in range(0, batch_size, chunk_size):
            input_chunk = input_uv[i:i+chunk_size].float().to(self.device)
            true_chunk = uv_true[i:i+chunk_size].float().to(self.device)

            pred_chunk = self.network(input_chunk, ele)

            loss_u = self.loss(pred_chunk, true_chunk, self.u)
            loss_v = self.loss(pred_chunk, true_chunk, self.v)
            loss_ssim = self.SSIM_loss(pred_chunk, true_chunk)
            loss_angle = self.Angle_loss(true_chunk, pred_chunk)
        ...
        return (total_loss_u / (batch_size/chunk_size),
                total_loss_v / (batch_size/chunk_size),
                total_loss_ssim / (batch_size/chunk_size),
                total_loss_angle / (batch_size/chunk_size),
                total_loss / (batch_size/chunk_size))
        ...
        with torch.no_grad():
            ...
            for input_uv, uv_true in dataloader_test:
                ...
                for i in range(0, batch_size, chunk_size):
                    ...
                    uv = self.network(input_chunk, ele)
                    uv_pred.append(uv)
    ...
    def infer(self, dataset, dataloader, ele):
        ...
        with torch.no_grad():
            ...
            for _, target in dataloader:
                uv_true_list.append(target)
            uv_true = torch.cat(uv_true_list, dim=0).float().to(self.device)
...
if __name__ == '__main__':
    ...
    class DataWrapper(Dataset):
        ...
        def split_data(self, test_size=0.1, random_state=35):
            ...
            train_dataset = SubsetDataWrapper(self.base_dataset, train_indices)
            test_dataset = SubsetDataWrapper(self.base_dataset, test_indices)

            return train_dataset, test_dataset

    class SubsetDataWrapper(Dataset):
        ...
        def GetDataShape(self):
            if len(self) > 0:
                data = self.base_dataset[self.indices[0]]
                return {'input': (len(self), 24, data.shape[1], data.shape[2], data.shape[3]),
                        'target': (len(self), 24, data.shape[1], data.shape[2], data.shape[3])}
            else:
                return {'input': (0, 0, 0, 0, 0),
                        'target': (0, 0, 0, 0, 0)}

    base_dataset = data_process(uv_train, samples_gap=3)
    dataset_train = DataWrapper(base_dataset)
    del uv_train
    ...
    trainer.train(dataset_train, dataset_val, ele, 'chkfile/checkpoint_ambhfn.chk')